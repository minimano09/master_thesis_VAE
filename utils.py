...
def gaussian_filter(kernel_size=5, sigma=1.0, channels=1):
    ...
    g = g / g.sum()
    kernel = torch.ger(g, g).unsqueeze(0).unsqueeze(0)  # Shape (1,1,H,W)
    
    # Expand kernel to match the number of input channels
    kernel = kernel.repeat(channels, 1, 1, 1)  # Shape (channels, 1, H, W)

    return kernel

def ssim_single_scale(img1, img2, k1=0.01, k2=0.03, filter_size=3, filter_sigma=1.0, L=1.0):
    channels = img1.shape[1]  # Get number of channels from the input image
    ...
    # Mean values
    mu1 = F.conv2d(img1, kernel, stride=1, padding=filter_size // 2, groups=channels)
    mu2 = F.conv2d(img2, kernel, stride=1, padding=filter_size // 2, groups=channels)
    
    # Variances and covariances
    mu1_sq = mu1 ** 2
    mu2_sq = mu2 ** 2
    mu1_mu2 = mu1 * mu2
    sigma1_sq = F.conv2d(img1 ** 2, kernel, stride=1, padding=filter_size // 2, groups=channels) - mu1_sq
    sigma2_sq = F.conv2d(img2 ** 2, kernel, stride=1, padding=filter_size // 2, groups=channels) - mu2_sq
    sigma12 = F.conv2d(img1 * img2, kernel, stride=1, padding=filter_size // 2, groups=channels) - mu1_mu2
...
def ms_ssim(img1, img2, power_factors=[0.0448, 0.2856, 0.3001, 0.2363, 0.1333], filter_size=3, filter_sigma=1.0, L=1.0):
    ...
    return torch.sum(torch.stack(msssim))
